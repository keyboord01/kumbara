...
export async function buildVaultDeposit(kit: SmartAccountKit, vaultId: string, address: string, amountUsdc: string) {
  const amount = nativeToScVal(usdcToStroops(amountUsdc), { type: "i128" });
  return buildContractCallTransaction(kit, {
    contractId: vaultId,
    method: "deposit",
    args: [xdr.ScVal.scvVec([amount]), xdr.ScVal.scvVec([amount]), Address.fromString(address).toScVal(), xdr.ScVal.scvBool(false)],
  });
}
