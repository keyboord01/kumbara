...
- **testanchor.stellar.org** now completes end to end. Its reference server answers per-transaction KYC only if the base customer is registered first and the full field set arrives in **one** `PUT /customer` carrying `transaction_id` *before the anchor's own check runs*, about two seconds after the `deposit-exchange`; answering the fixture before the request, or answering after the transaction has flipped to `pending_customer_info_update`, parks it there for good (a re-PUT never revives it). So the answer is sent inside the bridge's pre-lock hook, right after the request and before the lock takes its seconds. With that order the transaction goes `pending_customer_info_update` → `pending_user_transfer_start` with the account and routing number (+2 s) → `pending_anchor` "Funds received from user" (+5 s) → `completed`: the reference anchor simulates the wire itself and pays the quoted USDC (5 USD → 3.9216 USDC, exact match), so no sandbox hook is needed. Browser round trip through the app: request 16 s, bank-to-vault 30 s, 82 s in all, 3.92 USDC in the vault; CI's preview job runs it as a full round trip (`E2E_EXPECT=autofund`). Its limits are the SEP-6 ones (1–10 USDC) stated in USD at its own SEP-38 price, since it publishes no fiat limits.
