...
| 4 | Nothing: deposits worth up to $50 are confirmed by the driver; above that the presenter taps **Play the bank** on the row | Sandbox transfer matched; the app detects the lira, runs the landing-account on-ramp and moves the USDC into the kumbara | 35–55 s |
...
Small deposits need no press at all: while the admin tab (or the GitHub backstop) ticks, the driver confirms any deposit worth $50 or less (`BOOTH_AUTO_CONFIRM_MAX_USD`, default 50, compared in lira at the live rate, so about ₺2,400 today) about five seconds after the visitor reaches the IBAN screen, and the record notes `bankPlayed.by = auto`. Larger deposits show in the console's queue, each with its own **Play the bank** button (and a **Play all** button when several wait); the console's lead line names the current threshold. `BOOTH_AUTO_BANK_MAX_TRY=0` turns the automatic play off.
