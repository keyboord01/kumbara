...
**Public stats.** `/stats` is read-only and reads `/api/metrics` (cached 30 s) plus `/api/health`. Headline numbers carry a TESTNET badge, the live USDC-in-vault figure is read from the vault contract on every snapshot, addresses are truncated, there are no per-user timelines, timings show medians and p90 only with at least five samples (otherwise "not enough data"), and the sponsor appears as a fraction of its threshold, never as an address. `?mode=tv` is the projector variant. The funnel section counts, per pipeline stage, how many stored deposit and withdrawal records reached that stage (from each record's status history) and how many never reached the next one, plus terminal exits (failed, cancelled, abandoned); it follows the same window and source filter, so seed and E2E records stay out of the public numbers.
...
### Finding a kumbara again (recovery)

No third-party indexer. The smart-account kit deploys every kumbara at an address derived from sha256(passkey credential id) and a public, deterministic deployer, so the passkey that opened a kumbara finds it on any device by derivation alone: "Kumbaram zaten var" and the recovery screen (`/kurtar`) prompt the passkey once and connect to the derived address after checking its code hash on chain. Kumbara's own registry (`/api/registry`, table `credentials`) remembers sha256(credential id) → contract: the relay writes it from the deployment preimage when the account is created (verified, nothing trusted from the client), and the browser writes it, unverified, when a backup passkey is enrolled on the security screen, because a backup passkey cannot derive the address. The recovery screen tries derivation, then the registry, then an address the person types. The kit's default indexer (Mercury) stopped resolving in September 2026; `indexerUrl: false` in the wallet config turns it off, and the registry replaces the one thing it did for Kumbara.
