...
| `anchor_auth_failed` | SEP-10 challenge or token refused for the bridge account at request time (`anchor_auth_failed`), or the bridge token expired before the anchor settled (`anchor_auth_expired`) | The anchor did not recognise the bridge account. | Try again (a fresh bridge account) | Anchor down |
| `pending_trust` | SEP-6 transaction status `pending_trust` although the bridge account was created with the trustline | The anchor is waiting for a trustline. | none (presenter, anchor team) | Anchor down |
| `anchor_rejected` | Anchor 4xx on SEP-38 quote / SEP-6 request; pipeline `sep6_error`, `sep6_refunded`, `sep6_expired` | The anchor refused this request. | Change the amount | – |
